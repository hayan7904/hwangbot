...
const selectByKeyStmt = db.prepare(`SELECT value FROM variables WHERE key=?`);
const updateByKeyStmt = db.prepare(`UPDATE variables SET value=? WHERE key=?`);
const insertStmt = db.prepare(`INSERT INTO variables (key, value) VALUES (?, ?)`);
const deleteByKeyAndValueStmt = db.prepare(`DELETE FROM variables WHERE key=? AND value=?`)
